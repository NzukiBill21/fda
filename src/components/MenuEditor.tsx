...
const MenuEditor = ({ token, onClose }: MenuEditorProps) => {
  ...
  const handleSave = async (item: any) => {
    try {
      ...
      // If image file is uploaded, upload it to server first
      if (item.imageFile) {
        toast.loading('Uploading image...', { id: 'image-upload-update' });
        try {
          const formData = new FormData();
          formData.append('image', item.imageFile);
          
          const uploadRes = await fetch(createApiUrl('api/uploads/item-image'), {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${token}`
            },
          ...
          toast.success('Image uploaded successfully!', { id: 'image-upload-update' });
        } catch (error: any) {
          toast.error(`Failed to upload image: ${error.message}`, { id: 'image-upload-update' });
          // Fallback to existing image or preview
          imageUrl = item.imagePreview || item.image;
        }
      }
      ...
      const itemToSave = {
        ...item,
        image: imageUrl, // Use uploaded image URL or existing
      ...
      delete itemToSave.imagePreview;
      ...
      if (error.message?.includes('404') || error.message?.includes('not found')) {
        ...
        toast.error(`❌ Server error: ${error.message}. Check backend logs.`);
      } else {
                  ...
                  {filteredItems.map(item => (
                    ...
                    >
                      <div className="relative flex-shrink-0 overflow-hidden rounded-lg mb-3 bg-gray-100" style={{ height: '160px', width: '100%' }}>
                        <img
                          src={toImageSrc(item.image) || getFallbackImage(item.name, item.category)}
                          alt={item.name}
                          ...
                          onError={(e) => {
                            ...
                            const fallbackImage = getFallbackImage(item.name, item.category);
                  ...
                  <div className="space-y-3">
                    ...
                    {(newItem.imagePreview || newItem.image) && (
                      <div className="relative group">
                        <img
                          src={toImageSrc(newItem.imagePreview || newItem.image) || getFallbackImage(newItem.name, newItem.category)}
                          alt="Preview"
                          className="w-full h-48 object-cover rounded-lg border-2 border-gray-200"
                          onError={(e) => {
                            const target = e.target as HTMLImageElement;
                            const fallback = getFallbackImage(newItem.name, newItem.category);
