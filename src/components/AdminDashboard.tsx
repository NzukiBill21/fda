...
export function AdminDashboard({ token, setIsAuthOpen, onLogout }: AdminDashboardProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-950 via-orange-900 to-yellow-900 pt-6 pb-10 px-4 relative">
      {/* Mondas watermark */}
      {/* Watermark removed - invalid image path */}
      <div className="container mx-auto max-w-7xl">
        {/* Local top bar with logo */}
        <div className="flex items-center justify-between mb-3">
          ...
          <button 
            onClick={onLogout} 
            className="px-4 py-2 rounded-lg bg-white/15 text-white border border-white/30 hover:bg-white/25 transition-all font-semibold"
          >
            Logout
          </button>
        </div>
        ...
        {/* Orders Tab */}
        {/* Orders Tab - Full Orders Management */}
        {activeTab === 'orders' && (
          <div className="space-y-6">
            {!token ? (
              <div className="bg-white rounded-xl shadow-lg p-8 text-center">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">Authentication Required</h2>
                <p className="text-gray-600 mb-6">Please log in to view orders management</p>
                <button
        ...
        {/* Overview Tab - Recent Orders Summary */}
        {activeTab === 'overview' && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        ...
                        <p className="text-white/70 text-xs mt-2">
                          {order.createdAt ? (() => { const d = new Date(order.createdAt); return !isNaN(d.getTime()) ? d.toLocaleString() : 'N/A'; })() : 'N/A'}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-green-400 font-bold text-2xl">
                          KES {order.total != null && typeof order.total === 'number' ? order.total.toLocaleString('en-KE', { minimumFractionDigits: 0, maximumFractionDigits: 0 }) : '0'}
                        </p>
                        <motion.button
