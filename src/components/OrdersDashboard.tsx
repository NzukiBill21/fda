...
import { ORDER_STATUSES, STATUS_LABELS, STATUS_COLORS } from '../constants/orderStatuses';
import { 
  Package, 
  Clock, 
  MapPin, 
...
import { toImageSrc, getFallbackImage } from '../utils/imageUtils';
...
const OrdersDashboard: React.FC<OrdersDashboardProps> = ({ token, variant = 'default' }) => {
  ...
  const statusOptions = [
    ...
    { value: ORDER_STATUSES.PENDING, label: STATUS_LABELS[ORDER_STATUSES.PENDING] },
    { value: ORDER_STATUSES.CONFIRMED, label: STATUS_LABELS[ORDER_STATUSES.CONFIRMED] },
    { value: ORDER_STATUSES.PREPARING, label: STATUS_LABELS[ORDER_STATUSES.PREPARING] },
    { value: ORDER_STATUSES.READY, label: STATUS_LABELS[ORDER_STATUSES.READY] },
    { value: ORDER_STATUSES.OUT_FOR_DELIVERY, label: STATUS_LABELS[ORDER_STATUSES.OUT_FOR_DELIVERY] },
    { value: ORDER_STATUSES.DELIVERED, label: STATUS_LABELS[ORDER_STATUSES.DELIVERED] },
    { value: ORDER_STATUSES.CANCELLED, label: STATUS_LABELS[ORDER_STATUSES.CANCELLED] }
  ];
  ...
  const fetchOrders = async () => {
    try {
      ...
      if (result.success) {
        // Normalize orders data - ensure all required fields exist with defaults
        const normalizedOrders = (result.orders || []).map((order: any) => ({
          ...order,
          ...
          items: Array.isArray(order.items) ? order.items.map((item: any) => ({
            ...
            menuItem: {
              ...item.menuItem,
              price: item.menuItem?.price != null ? parseFloat(item.menuItem.price) : (item.menuItemPrice != null ? parseFloat(item.menuItemPrice) : 0),
              name: item.menuItem?.name || item.menuItemName || 'Unknown Item',
              image: item.menuItem?.image || item.menuItemImage || null
            }
          })) : [],
  ...
  const fetchStats = async () => {
    try {
      ...
      if (result.success) {
        // Normalize stats - ensure all numeric fields have defaults
        const normalizedStats = {
          totalOrders: result.stats?.totalOrders != null ? parseInt(result.stats.totalOrders) : 0,
          pendingOrders: result.stats?.pendingOrders != null ? parseInt(result.stats.pendingOrders) : 0,
          confirmedOrders: result.stats?.confirmedOrders != null ? parseInt(result.stats.confirmedOrders) : 0,
          preparingOrders: result.stats?.preparingOrders != null ? parseInt(result.stats.preparingOrders) : 0,
          outForDeliveryOrders: result.stats?.outForDeliveryOrders != null ? parseInt(result.stats.outForDeliveryOrders) : 0,
          deliveredOrders: result.stats?.deliveredOrders != null ? parseInt(result.stats.deliveredOrders) : 0,
          cancelledOrders: result.stats?.cancelledOrders != null ? parseInt(result.stats.cancelledOrders) : 0,
          totalRevenue: result.stats?.totalRevenue != null ? parseFloat(result.stats.totalRevenue) : 0,
          todayOrders: result.stats?.todayOrders != null ? parseInt(result.stats.todayOrders) : 0,
          thisWeekOrders: result.stats?.thisWeekOrders != null ? parseInt(result.stats.thisWeekOrders) : 0
        };
  ...
  useEffect(() => {
    ...
    // Real-time polling for live updates (every 5 seconds)
  ...
  }, [currentPage, filterStatus, token]); // Added token to dependencies for proper re-fetching
      ...
      <html>
        ...
        <body>
          ...
          <div class="stats">
            ...
            <div class="stat-box">
              <span class="kpi-label">Total Revenue:</span> <span class="kpi-value">KES ${stats.totalRevenue != null && typeof stats.totalRevenue === 'number' ? stats.totalRevenue.toLocaleString('en-KE') : '0'}</span>
            </div>
          </div>
          ` : ''}
            ...
            <tbody>
              ${filteredOrders.map(order => `
                <tr>
                  ...
                  <td>${order.status}</td>
                  <td>${order.total != null && typeof order.total === 'number' ? order.total.toLocaleString('en-KE') : '0'}</td>
                  <td>${order.items.map(item => `${item.quantity}x ${item.menuItem.name}`).join(', ')}</td>
                  <td>${order.createdAt ? (() => { const d = new Date(order.createdAt); return !isNaN(d.getTime()) ? d.toLocaleString() : 'N/A'; })() : 'N/A'}</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
  ...
  const exportToPNG = async () => {
    try {
      ...
      try {
        ...
        // Skip logo - image path invalid in production
      ...
      if (stats) {
        ...
        const kpis = [
          ...
          { label: 'Total Revenue', value: `KES ${stats.totalRevenue != null && typeof stats.totalRevenue === 'number' 
            ? stats.totalRevenue.toLocaleString('en-KE') 
            : '0'}` },
          { label: 'Avg Order Value', value: `KES ${aov != null && typeof aov === 'number' 
            ? aov.toLocaleString('en-KE') 
            : '0'}` },
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">KES {stats.totalRevenue != null && typeof stats.totalRevenue === 'number' 
                  ? stats.totalRevenue.toLocaleString('en-KE', { minimumFractionDigits: 0, maximumFractionDigits: 0 })
                  : '0'}</p>
              </div>
              ...
              {filteredOrders.length === 0 ? (
                ...
                <tr key={order.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    KES {order.total != null && typeof order.total === 'number' 
                      ? order.total.toLocaleString('en-KE', { minimumFractionDigits: 0, maximumFractionDigits: 0 })
                      : '0'}
                  </td>
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-3">
                    {selectedOrder.items.map((item) => {
                      const imageSrc = toImageSrc(item.menuItem?.image || item.menuItemImage) || 
                        getFallbackImage(item.menuItem?.name || item.menuItemName, item.menuItem?.category);
                      return (
                      <div key={item.id} className="flex items-center space-x-4 p-3 bg-gray-50 rounded-xl">
                        <div className="w-12 h-12 bg-gradient-to-br from-orange-400 to-red-500 rounded-lg flex items-center justify-center overflow-hidden">
                          <img
                            src={imageSrc}
                            alt={item.menuItem?.name || item.menuItemName || 'Item'}
                        ...
                        <div className="flex-1">
                          <h4 className="font-medium text-gray-900">{item.menuItem?.name || item.menuItemName || 'Unknown Item'}</h4>
                          <p className="text-sm text-gray-600">KES {item.menuItem?.price != null && typeof item.menuItem.price === 'number'
                            ? item.menuItem.price.toLocaleString('en-KE')
                            : (item.price != null && typeof item.price === 'number' ? item.price.toLocaleString('en-KE') : '0')} × {item.quantity || 0}</p>
                        </div>
                        <div className="text-right">
                          <div className="font-bold text-gray-900">
                            KES {(() => {
                              const price = item.menuItem?.price != null && typeof item.menuItem.price === 'number' 
                                ? item.menuItem.price 
                                : (item.price != null && typeof item.price === 'number' ? item.price : 0);
                              const qty = item.quantity != null ? item.quantity : 0;
                              return (price * qty).toLocaleString('en-KE', { minimumFractionDigits: 0, maximumFractionDigits: 0 });
                            })()}
                          </div>
                        </div>
                      </div>
