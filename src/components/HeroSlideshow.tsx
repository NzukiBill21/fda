...
export function HeroSlideshow({ onAddToCart, onOpenCart, onDismiss }: HeroSlideshowProps) {
  ...
  useEffect(() => {
    const fetchMenuItems = async () => {
      try {
        ...
        const createApiUrl = apiModule.createApiUrl || ((url: string) => `http://localhost/mondas-api/${url}`);
  ...
  useEffect(() => {
    if (!slides || slides.length === 0) return; // Guard against empty slides
    
    const timer = setInterval(() => {
      setDirection(1);
      setCurrentSlide((prev) => {
        // Ensure prev is valid before calculating next
        const safePrev = prev >= 0 && prev < slides.length ? prev : 0;
        const next = (safePrev + 1) % slides.length;
  ...
  const paginate = (newDirection: number) => {
    ...
    if (!slides || slides.length === 0) {
      console.warn('Cannot paginate: slides array is empty');
      return;
    }
    ...
    setCurrentSlide((prev) => {
      ...
      const safePrev = prev >= 0 && prev < slides.length ? prev : 0;
      const next = safePrev + newDirection;
      
      // Wrap around safely
      if (next < 0) return slides.length - 1;
      if (next >= slides.length) return 0;
      
      // Ensure the next slide exists
      if (slides[next]) {
        return next;
      }
      
      // Fallback to 0 if next slide doesn't exist
      return 0;
    });
  };
  ...
  const handleOrderNow = () => {
    ...
    const currentOffer = slides[safeIndex];
    if (!currentOffer) return; // Guard against undefined
    
    onAddToCart({
      id: `offer-${currentOffer.id}`,
      name: currentOffer.title || 'Special Offer',
      description: currentOffer.description || '',
      price: currentOffer.priceValue || 0,
  ...
  if (!slides || slides.length === 0) {
    return (
      <div className="relative h-[75vh] w-full overflow-hidden bg-gradient-to-br from-red-950 via-red-900 to-yellow-900 flex items-center justify-center">
        <p className="text-white text-lg">Loading offers...</p>
      </div>
    );
  }

  // Ensure currentSlide is valid - use useEffect to fix invalid state
  useEffect(() => {
    if (slides.length > 0) {
      if (currentSlide < 0 || currentSlide >= slides.length || !slides[currentSlide]) {
        setCurrentSlide(0);
      }
    }
  }, [currentSlide, slides]);

  // Calculate safe current slide for rendering - ENSURE IT'S ALWAYS VALID
  const safeCurrentSlide = slides.length > 0 
    ? (currentSlide >= 0 && currentSlide < slides.length && slides[currentSlide] ? currentSlide : 0)
    : 0;
  
  // CRITICAL: Ensure we always have valid slide data
  const currentSlideData = slides.length > 0 && slides[safeCurrentSlide] ? slides[safeCurrentSlide] : (slides.length > 0 ? slides[0] : null);

  if (!currentSlideData || !slides || slides.length === 0) {
    // If no valid slide data, show loading/empty state
    return (
      <div className="relative h-[75vh] w-full overflow-hidden bg-gradient-to-br from-red-950 via-red-900 to-yellow-900 flex items-center justify-center">
        <p className="text-white text-lg">{slides.length === 0 ? 'No offers available' : 'Loading offers...'}</p>
      </div>
    );
  }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 md:gap-8 lg:gap-12 items-center w-full">
              ...
              >
                ...
                <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl mb-2 sm:mb-3 md:mb-4 text-white drop-shadow-2xl leading-tight tracking-tight font-extrabold">
                  {currentSlideData?.title || 'Delicious Food'}
                </h1>
                ...
                <p className="text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl text-yellow-100 mb-3 sm:mb-4 md:mb-6 drop-shadow-lg leading-relaxed">
                  {currentSlideData?.description || 'Fresh and delicious meals delivered to your door'}
                </p>
              ...
              >
                <div className="relative w-full h-full rounded-lg sm:rounded-xl md:rounded-2xl lg:rounded-3xl overflow-hidden shadow-xl border-2 sm:border-3 md:border-4 lg:border-8 border-white/10">
                  <img
                    src={toImageSrc(currentSlideData?.image) || getFallbackImage(currentSlideData?.title || 'Special Offer', 'Special Offers')}
                    alt={currentSlideData?.title || 'Special Offer'}
                    ...
                    onError={(e) => {
                      ...
                      const fallback = getFallbackImage(currentSlideData?.title || 'Special Offer', 'Special Offers');
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (index >= 0 && index < slides.length && slides[index]) {
                        setDirection(index > safeCurrentSlide ? 1 : -1);
